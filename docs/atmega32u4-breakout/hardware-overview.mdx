...
a clock switch over can be executed in the user application transitioning the clock to be driven from an external crystal connected to ```XTAL1``` and ```XTAL2```.
- ```XTAL1```: External Crystal pin 1
- ```XTAL2```: External Crystal pin 2

:::note
If an external crystal is used, the frequency must be between 8 and 16MHz
:::
...
:::info
More information on programming and bootloading the device can be found in the [programming](programming) docs.
:::