...
To program the breakout board via SPI you can connect to the dedicated programming header which is the same signals as the ```B0``` - ```B4``` GPIO pins which have the ```SPI``` alternate function.