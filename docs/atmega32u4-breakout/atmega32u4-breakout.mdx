...
### Special & Programming
The special pins are generally used for programming the device via JTAG, SPI or USB DFU. You can also use the ```nRST``` pin in general use to reset the MCU in the case the application hangs or a larger system reset
...
To program the breakout board via SPI you can connect to the dedicated programming header which is the same signals as the ```B0``` - ```B4``` GPIO pins with the ```SPI``` alternate function. To initiate USB bootloading
the ```nHWB``` pin can be held ```LOW``` while momentarily pulling ```nRST``` low and then releasing it.