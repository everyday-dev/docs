...
To program the breakout board via SPI you can connect to the dedicated programming header or you can use the ```B0``` - ```B4``` GPIO pins which have the ```SPI``` alternate function.