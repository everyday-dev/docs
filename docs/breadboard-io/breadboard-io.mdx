...
## Introduction
breadboard-io makes it easy to connect 4 LEDs and 4 push-buttons to a broadboard in a compact and neat footprint. Provide VCC, GND and your signal wires to other circuitry or microcontroller and you're ready to go!
...
## Hardware Overview
The breadboard IO provides the user access to the following via a single 2.54" standard pin header:
...
Ex: ```SW1``` is normally at GND but when the ```SW1``` push-button is pressed, the ```SW1``` pin will go from GND to VCC.

Ex: To turn on ```LED1```, you would apply a high or positive signal (VCC) to the ```LED1``` pin which then illuminates ```LED1```.
:::
