...
description: Guide on how to flash the bare-metal USB DFU bootloader onto the atmega32u4 breakout board by great lakes dev
# image: tbd
---
## Introduction
If you have an atmega32u4 breakout board with the ```Arduino``` bootloader (or empty flash) and would like to use the board with the [avr-cmake-template](https://github.com/greatlakesdev/avr-cmake-template)
...
To write the USB-DFU bootloader we need to retrieve the ```.hex``` from [this Github repository](https://github.com/greatlakesdev/megaAVR_USB_DFU_firmware). The ```.hex```
