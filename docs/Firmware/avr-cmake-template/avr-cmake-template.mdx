...
:::info
The template can be found on Github in the [avr-cmake-template](https://github.com/greatlakesdev/avr-cmake-template) repository
:::
...
- Install the ```choco``` package manager for Windows if you haven't already
...
- Disable ```choco``` global confirmation to avoid having to confirm the installation of individual programs:
```console
...
```
- Install the ```avr-gcc``` toolchain provided on the [Microchip site](https://www.microchip.com/en-us/tools-resources/develop/microchip-studio/gcc-compilers). The extracted toolchain can be placed wherver you like. Once extracted add the ```bin``` folder path to your Path environment variable.
  - Verify ```avr-gcc``` is available from the CLI once completed.
...
<Tabs groupId="operating-systems">
  ...
  ```
  - Download and install [dfu-programmer](https://github.com/dfu-programmer/dfu-programmer/releases/tag/v1.0.0) from the ```.tar.gz``` provided in the Releases section
    - Ensure ```dfu-programmer``` is available from the command line by adding it to your ```$PATH``` environment variable
...
:::
  ...
  ```console
      ...
      python -m pip install gcovr
  ```
  - Download and install [dfu-programmer](https://github.com/dfu-programmer/dfu-programmer/releases/tag/v1.0.0) from the ```x86``` or ```x64``` zip files
...
Note that this is a powershell script and should be executed from Powershell.
:::note
If you have never run powershell scripts on your machine before, you will likely need to allow remote signed scripts. To do so open Poweshell as an administrator and execute the following
```console
...
Once a ```build/``` directory has been initialized the following ```make``` commands can be ran from the ```build/``` folder
...
<Tabs groupId="prog-type">
  <TabItem value="programmer" label="avr-dude / programmer" default>
...
#### Fuses
:::note
Reading and writing fuses is only supported when connected to the target via avrdude & programmer
:::
To write the fuses:
```console
make write_fuses
