...
## Tools Setup
There are a handful of tools needed to use this template. Several are required such as the AVR toolchains and others are not, such as the Ceedling unit testing framework. We will outline
the install of the required and nice to have tools in seperate steps.
...
```
Once you add the new source & unit-test file is created, don't forget to add the newly created ```src/*.c``` file to the ```APP_SRC``` variable in ```CMakeLists.txt```
